...
int minTime() {
    ...
    for(int source = 0; source < N; source++) {
        ...
        while(!pq.empty()) {
            ...
            if(visited[where]) continue; // kiểm tra xe này đã xét chưa, bến đỗ này xét chưa
            ...
            if(where < N) { // Nếu nó là xe thì
                // cout << where << " is a car" << endl;
                for(int i = 0; i < M; i++) { // Xét các bến đỗ 
            ...
            // Nếu là chỗ đỗ xe
                ...
                // Nếu chỗ đỗ trống
                if(park_match[where - N] == -1) { // Nếu chỗ đỗ xe này chưa có xe 
    ...
    // }
    // for(int i = 0; i < M; i++) {
    //     if(park_match[i] == -1) cout << "parking slot number " << i << " has no car" << endl;
    //     else cout << "parking slot number " << i << " have car number " << park_match[i] << endl;
    // }
    return D;
}
