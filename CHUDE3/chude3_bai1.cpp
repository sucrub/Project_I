...
struct Edge // Một cạnh gồm 2 đỉnh và trọng số
{

	int u, v, weight;
};

void generate_graph(Edge *mt) // Tạo đồ thị ngẫu nhiên
{

	srand(time(NULL));

	int **temp = new int *[N];
	for (int i = 0; i < N; i++) // Lưu tạm giá trị để xét
	{
		temp[i] = new int[N];
	}
	for (int i = 0; i < N; i++)
	{
		for (int j = 0; j < N; j++)
		{
			if (i == j) // Hai đỉnh trùng thì skip
				continue;
			if (i > j) // Đỉnh i > j thì lấy lại giá trị của cạnh j i đã xét trước đây
			{
				mt[index].u = i;
				mt[index].v = j;
...
void printSolution()
{
	...
	for (int i = 0; i < N; i++)
	{
		...
		if (dist_no_priority[i] != dist_priority[i])
			check_solution = 1;
	}
	if (check_solution == 0)
...
void dijkstra_no_priority(Edge *graph)
{
	...
	for (int count = 0; count < N - 1; count++)
	{
		...
		for (int v = 0; v < N - 1; v++)
			...
			if (!check_no_priority[graph[u * N - u + v].v] && dist_no_priority[graph[u * N - u + v].u] != 5000 && dist_no_priority[graph[u * N - u + v].u] + graph[u * N - u + v].weight < dist_no_priority[graph[u * N - u + v].v])
